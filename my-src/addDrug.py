...
"background-color: rgb(166, 0, 0)")
        ...
        ##################### ปุ่มในหน้าเพิ่มยา #######################
        self.add_back_pushButton.clicked.connect(close_window)          # เชื่อมต่อกับฟังก์ชัน close_window ใช้ปิดหน้าต่าง
        self.pushButton.clicked.connect(self.save_drug)                 # เชื่อมต่อกับฟังก์ชัน save_drug ใช้บันทึกชื่อยา
        self.delete_pushButton.clicked.connect(self.delete_drug)        # เชื่อมต่อกับฟังก์ชัน delete_drug ใช้ลบรายการยา
    ...
    def save_drug(self):
        ...
        ############################ ปุ่มและแจ้งเตือนไม่มีชื่อยา ############################
        if not drug_name.strip():
            # สร้าง QDialog เพื่อแจ้งเตือนว่าไม่ได้ใส่ชื่อยา
            ...
            # ตั้งค่าสีพื้นหลังของไม่มีชื่อยา
            error_dialog.setStyleSheet("background-color: rgb(255, 241, 129); border-radius: 30px;")

            # ตั้งค่าสีข้อความและปุ่มของไม่มีชื่อยา
            ok_button.setStyleSheet("background-color: rgb(85, 170, 127); color: white; border: none; border-radius: 15px;")

            # เลือกตำแหน่งของไม่มีชื่อยา
            layout = QtWidgets.QVBoxLayout()
            layout.addWidget(error_label)
            layout.addWidget(ok_button)
            layout.setContentsMargins(40, 30, 40, 30)       # เพิ่ม margins สำหรับ layout (left, top, right, bottom)
            error_dialog.setLayout(layout)

            # ปุ่ม OK เพื่อปิดแจ้งเตือนไม่มีชื่อยา
            ok_button.clicked.connect(error_dialog.accept)

            # แสดง QDialog ของไม่มีชื่อยา
            error_dialog.exec_()
            return  # Do not proceed with saving

        ############################# เชื่อมต่อฐานข้อมูลเพื่อเพิ่มชื่อยา #############################
        ...
        ############################ ปุ่มและแจ้งเตือนบันทึกชื่อยา ############################
        # สร้าง QDialog เพื่อแจ้งเตือนว่าบันทึกสำเร็จ
        ...
        # ตั้งค่าสีพื้นหลังของบันทึกชื่อยา
        message_dialog.setStyleSheet("background-color: rgb(255, 241, 129); border-radius: 30px;")

        # ตั้งค่าสีข้อความและปุ่มของบันทึกชื่อยา
        ok_button.setStyleSheet("background-color: rgb(85, 170, 127); color: white; border: none; border-radius: 15px;")

        # เลือกตำแหน่งของบันทึกชื่อยา
        layout = QtWidgets.QVBoxLayout()
        layout.addWidget(message_label)
        layout.addWidget(ok_button)
        layout.setContentsMargins(50, 30, 50, 30)       # เพิ่ม margins สำหรับ layout (left, top, right, bottom)
        message_dialog.setLayout(layout)

        # ปุ่ม OK เพื่อปิดแจ้งเตือนบันทึกชื่อยา
        ok_button.clicked.connect(message_dialog.accept)

        # แสดง QDialog ของบันทึกชื่อยา
        message_dialog.exec_()
        
        # ล้างข้อมูลใน textEdit และ textEdit_2 หรือล้างข้อความชื่อช่องใส่ชื่อยาและคำอธิบายยาหลังการปุ่มปิดแจ้งเตือนบันทึกชื่อยา
        self.textEdit.clear()
    ...
    def delete_drug(self): 
        ...
        if selected_item:
            ...
            # สร้างปุ่ม ใช่ และ ไม่ ในการลบชื่อยา
            ...
            if result == QDialog.Accepted:
                connection = sqlite3.connect("medicine.db")
                cursor = connection.cursor()
                cursor.execute("DELETE FROM Drug WHERE drug_name=?", (drug_name,))      # ลบรายการยาในฐานข้อมูล
                connection.commit()
                connection.close()
                self.load_drug_list()   # แสดงรายการยาในปัจจุบันทั้งหมด

    ########################## โหลดยาทั้งหมด #########################
    def load_drug_list(self):
        connection = sqlite3.connect("medicine.db")
        cursor = connection.cursor()
        cursor.execute("SELECT drug_name FROM Drug")        # แสดง ชื่อยา จากตาราง Drug
        drugs = cursor.fetchall()
        connection.close()

        self.listWidget.clear()     # ล้างรายการใน Widget ที่แสดงในการรายการชื่อยา
